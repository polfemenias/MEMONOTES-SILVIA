...
// Component auxiliar per al textarea amb auto-resize i ESTAT LOCAL
// Això evita re-renders massius a cada lletra, solucionant el problema del focus.
const SubjectContentEditor: React.FC<{
    ...
    // Sincronitzar si el valor extern canvia (ex: carrega inicial)
    useEffect(() => {
        setLocalValue(value);
    }, [value]);
    ...
    const handleTranscript = (text: string) => {
        ...
        onChange(newValue); // Aquí sí guardem directe perquè és una acció puntual
    };
    ...
    return (
        <div className="relative mt-2">
            <textarea
                ...
                placeholder="Continguts treballats (es mostren 8 línies per defecte)..."
            />
...
// Extret completament fora per estabilitat
...
// Extret fora de SettingsPage per evitar re-renders que fan perdre el focus
const CourseContentEditor: React.FC<{
    ...
    onUnassignSubjectFromCourse,
    onUpdateCourseSubjectContent,
    onAssignSubjectToCourse
}) => {
    ...
    const handleUpdateStudentName = (student: Student, newName: string) => {
        onUpdateStudent({ ...student, name: newName });
    };
      ...
      <div className="mt-4 pt-4 border-t-2 border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 border rounded-lg bg-white">
            <EditableListItem isAdding onAdd={(name) => onAddClassGroup(course.id, name)} addLabel="Afegir nova classe (Ex: GRUP A)" />
          </div>
        </div>
        ...
        <div className="space-y-4">
              ...
              return (
                <div key={cs.subjectId} className="p-3 bg-white rounded-md border shadow-sm">
                  <div className="flex justify-between items-start">
                    ...
                    <button onClick={() => onUnassignSubjectFromCourse(course.id, cs.subjectId)} className="text-red-500 hover:text-red-700 p-1" title="Desassignar">
                  ...
                  <SubjectContentEditor 
                    ...
                    onChange={(val) => onUpdateCourseSubjectContent(course.id, cs.subjectId, val)} 
                  />
                </div>
              );
            })}
          ...
          <div className="mt-4 pt-4 border-t">
            <div className="flex items-center gap-2">
              <select
                value={subjectToAssignId}
                onChange={(e) => setSubjectToAssignId(e.target.value)}
...
const SettingsPage: React.FC<SettingsPageProps> = (props) => {
  ...
  // Evitem mutar props.courses directament amb sort()
              ...
              <div key={course.id} className="border p-4 rounded-lg bg-slate-100 shadow-sm">
                ...
                {activeCourseId === course.id && (
                    <CourseContentEditor
                        ...
                        onUpdateCourseSubjectContent={props.onUpdateCourseSubjectContent}
